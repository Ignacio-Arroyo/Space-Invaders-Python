...
class Fleet(object):
    ...
    def install_in(self, canvas):
        ...
        for i in range(0,self.fleet_size):
            ...
            self.aliens_fleet[i].install_in(canvas,50+(alien_width+self.aliens_inner_gap)*(i%10),50+(alien_height+self.aliens_inner_gap)*(i//10))
...
class Game(object):
    ...
    def get_score(self):
        self.score_act=(self.fleet.fleet_size-len(self.fleet.aliens_fleet))*100
        texte="Score: "+str(self.score_act)
        if self.score_text!=None:
            self.canvas.delete(self.score_text)
        self.score_text=self.canvas.create_text(100, 50, text=texte, fill="green", font=('Helvetica 25 bold'))
    def animation(self):
        self.get_score()
        if(self.isWon==None):
            self.fleet.move_in()
    ...
    def menu(self):
        ...
        self.background()
        def new_game():
            label=tk.Label(self.canvas, text="Entrer votre nom", font=("Arial", 17), fg='green', bg='black')
            self.canvas.create_window(self.max_w//2-100,230,window=label)
            self.name_entry = Entry(self.canvas,width=30)
            self.canvas.create_window(self.max_w/2-100,260,window=self.name_entry)
        def getname():
        ...
        txt_ng="New game"
        label=tk.Button(self.canvas, text=txt_ng, font=("Segoe UI", 15), fg='green', bg='white',borderwidth="0",command=getname)
        self.canvas.create_window(self.max_w//2-100,300,window=label)
        txt_quit="Quit"
        label=tk.Button(self.canvas, text=txt_quit, font=("Impact", 30), fg='green', bg='white',borderwidth="0",command=self.frame.quit)
        self.canvas.create_window(self.max_w//2,600,window=label)
        label=tk.Label(self.canvas, text="High Scores", font=("Impact", 20), fg='green', bg='white')
        self.canvas.create_window(self.max_w//2+160,250,window=label)
        y=i=0
        score=self.high_scores
        def high(y,i):
            try:
                label=tk.Label(self.canvas, text=str(score[i].player)+":"+str(score[i].score), font=("Segoe UI", 15), fg='white', bg='black')
                self.canvas.create_window(self.max_w//2+160,300+y,window=label)
                if(i<4):
